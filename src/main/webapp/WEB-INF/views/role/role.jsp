		...
		<div class="main-content">
			...
			<div class="page-content">
				<div class="page-header">
					<h1>
						系统管理 <small> <i class="icon-double-angle-right"></i> 角色
							&amp; 列表
						</small>
					</h1>
					<div class="col-sm-6" style="width: 100%">
					
						<a class="btn btn-info" href="role_input.action">新建 <i
							class="icon-plus "></i></a>
						&emsp;
						角色名：
						<s:textfield name="baseQuery.name" size="15"
							cssClass="nav-search-input" placeholder="请输入角色名称" />
<!-- 						部门： -->
										...
										<tbody>
											<s:iterator value="pageList.rows">
												<tr <s:if test="id==#parameters['id'][0]">style="color:red"</s:if> >
													...
													<td>${name}</td>
													<td>
														<s:iterator value="permissions" status="sta">
														   <s:if test="#sta.last">
														     ${name}
														   </s:if>
														   <s:else>
													...
													<td>
														<s:iterator value="menus" status="sta">
														   <s:if test="#sta.last">
														     ${name}
														   </s:if>
														   <s:else>
													...
													<td>
														<div
															...
															<button type="button"
																onclick="updateDomain('role',${id})"
																class="btn btn-xs btn-info">
																<i class="icon-edit bigger-120"></i>
															</button>
	
															<button type="button"
																onclick="deleteDomain(this,'role_delete.action?id=${id}')"
